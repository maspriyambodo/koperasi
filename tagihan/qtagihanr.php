<?php $bln=$result->c_d($_GET['bln']);$thn=$result->c_d($_GET['thn']);$kkbayar =$result->c_d($_GET['kkbayar']);$tabel = $tabel_tagihan.$bln.substr($thn,2,2).'b';$tabelr= $userid;$branch=$result->c_d($_GET['branch']);
if($branch=='0111'){
	...
	if($kkbayar=='9'){
		...
		$result->query_x1("CREATE TEMPORARY TABLE $tabelr(SELECT a.norek,SUM(a.pokok) as pokok,SUM(a.bunga) as bunga,SUM(a.adm) as adm,SUM(a.jumlah) as jumlah,count(*) as rekening,a.kdtran,b.kdbyr,b.kkbayar,b.nmbayar,b.kdsales FROM $tabel a JOIN $tabel_kredit b ON a.norek=b.norek WHERE b.kkbayar='$kkbayar' AND a.branch='$branch' GROUP BY b.kdbyr,b.kkbayar ORDER BY b.kdbyr,b.kkbayar)");
	}
}
