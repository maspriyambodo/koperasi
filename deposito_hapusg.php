...
<script type="text/javascript">
	$(document).ready(function () {
		$("#kwitansi").submit(function() {
			var r = confirm("Anda Yakin Pilihan Benar..?");
			if (r == false) {
				return false;
			}
			$.ajax({
				type: "POST",
				url : "deposito_hapuso.php",
				data: $(this).serialize(),
				beforeSend: function () {
					$('#loading').show();
				},
	...
	function goKembalii() {
		var id= $("#idd").val();var p= $("#pp").val();
		var dataString='id='+id+'&p='+p;
		$.ajax({
			type: "GET",
			url : "deposito_hapusg.php",
			data: dataString,
			beforeSend: function () {
				$('#loading').show();
			},
...
<?php
$id=$result->c_d($_GET['id']);$p=$result->c_d($_GET['p']);
echo '<input name="idd" type="hidden" id="idd" value="'.$id.'"/><input name="pp" type="hidden" id="pp" value="'.$p.'"/>';
$tabel="deposito.deposits_cadangan";$hasil=$result->query_lap("SELECT a.branch,a.nomor_nasabah,a.sufix,a.nomor_bilyet,a.seri_bilyet,a.tanggal_buka,a.tanggal_jatuh_tempo,a.nama_rekening,a.jangka_waktu,a.nominal,a.counter_rate,a.status_rekening,b.nmproduk,c.nama AS namas FROM deposito.deposits a JOIN deposito.prd_deposito b ON a.produk=b.kdproduk JOIN nabasa.sales c ON a.sales_id=c.idsales WHERE a.id='$id' LIMIT 1");
$row=$result->row($hasil);if($row['status_rekening']==4) die('Rekening Deposito Sudah Di Tutup');
$branch=$row['branch'];
...
while ($row=$result->row($hasil)){
	...
	<td width="6%" align="center"><input type="checkbox" name="id[]" '.$chek.' value="'.$row["id"].'"></td>
...
<td align="center" colspan="2"><input type="submit" value="'.$ket.'" id="submit" class="icon-proses"/></td>
