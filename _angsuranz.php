...
$jum=$jumlah;
if($jum>$bungakk){
	$jum=$jum-$bungakk;$hasilx=$result->query_y1("SELECT branch,norek,sufix,nama,nonas,produk,angsurke,kdtran,kdkop,bulan,tanggal,SUM(if(kdtran=111,pokok,0))-SUM(if(kdtran=777,pokok,0)) as pokok,SUM(if(kdtran=111,bunga,0))-SUM(if(kdtran=777,bunga,0)) as bunga,SUM(if(kdtran=111,adm,0))-SUM(if(kdtran=777,adm,0)) as adm FROM payment WHERE norek='$norek' GROUP BY angsurke ORDER BY angsurke");
	if($result->num($hasilx)<1)die("Data Tagihan Tidak Ada...?");
while($r=$result->row($hasilx)){
	$pokok=$r['pokok'];$bunga=$r['bunga'];$adm=$r['adm'];$xbranch=$r['branch'];$xnorek=$r['norek'];$xsufix=$r['sufix'];$xnonas=$r['nonas'];$xproduk=$r['produk'];$xangsurke=$r['angsurke'];$xbulan=$r['bulan'];$xnama=$r['nama'];$kdkop=$r['kdkop'];$tgl1=$r['tanggal'];
