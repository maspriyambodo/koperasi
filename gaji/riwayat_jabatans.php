...
case "1":
	$hasil=$result->query_y("SELECT MAX(no_index) AS no_index FROM $tabel_riwayat_ja WHERE nik='$nik'",'');
	...
	$lastID = $result->insert($tabel_riwayat_ja, $data);
	$result->close();
	echo 'Sukses Simpan Riwayat Kerja '.$nik;
	$catat="Menambah Data Riwayat Kerja ".$nik.' '.$userid;
	include '../around.php';
	break;
case "2":
    ...
    $where = array('id' => $id);
    $result->update($tabel_riwayat_ja, $data, $where);
	$result->close();
	echo 'Sukses Update Riwayat Kerja '.$nik;
	$catat="Update Data Riwayat Kerja ".' '.$userid;
	include '../around.php';
	break;
case "3":
	$id=$result->clean_data($_GET['id']);
    $where = array('ID' => $id);
    $result->delete($tabel_riwayat_ja, $where,1);
	$result->close();
	echo "Sukses Dihapus";
	break;
}
