...
(function($) {
	function lookupbox(options) {
		var settings = {
			...
			searchTextCabang: "lookupbox-search-cabang",
			searchResultId: "lookupbox-search-result",
			kodecabang: "",
			width: 750,
			htmlForm: '<form id="lookupbox-search-form" onsubmit="return false"><table><tr><td>Kriteria : <input id="lookupbox-search-cabang" type="text" name="cabang" placeholder="cabang" size="6" maxlength="4" autocomplete="Off" onKeyUp="caps(this)"/><input id="lookupbox-search-key" type="text" name="key" placeholder="nonas,nama" autocomplete="Off" onKeyUp="caps(this)"/><input type="button" id="lookupbox-search-button" value="Search" class="icon-search"/><span id="loading1"></span></td></tr></table></form><div id="lookupbox-search-result"></div>',
			modal: true,
			draggable: true,
			onItemSelected: null,
			item: null,
		...
		$(document).ready(function() {
			...
			$('input[name=cabang]').val(settings.kodecabang);			
			$("#" + settings.searchButtonId).click(function(){
				if (settings.onSearch == null) {
					$.ajax({
						...
						url: settings.url+$("#"+settings.searchTextId).val()+"&cabang="+$("#" + settings.searchTextCabang).val(),
