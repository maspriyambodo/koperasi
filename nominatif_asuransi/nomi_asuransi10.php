...
if($ada==TRUE){
	...
	if($branch=='0111'){
		if($produk==9){
			$hasil=$result->query_lap("SELECT a.norek,a.nonas,a.branch,c.kkbayar,c.nmbayar,sum(c.nomi) as nomi,c.jangka,c.suku,a.tgtran,c.produk,sum(a.saldo_awal) as saldo_awal,sum(a.mutasi_debet) as mutasi_debet,sum(a.mutasi_kredit) as mutasi_kredit,sum(a.saldo_akhir) as saldo_akhir,c.kdkop,c.kolek,b.nama,d.nmproduk,count(*) as counter FROM $tabel a JOIN $tabel_nasabah b ON a.nonas=b.nonas JOIN $tabel_kredit c ON a.norek=c.norek JOIN debit1 d ON c.produk=d.kdproduk WHERE (a.saldo_awal!=0 or a.mutasi_debet!=0 or a.mutasi_kredit!=0 or a.saldo_akhir!=0) GROUP BY c.produk,c.kkbayar ORDER BY c.produk,c.kkbayar");
		}else{
