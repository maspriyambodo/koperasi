...
function goKembali() {
	var url='./daftar_susulan44.php';
	$('#innertub').load(url);
}
</script>
<?php
$norek=$result->c_d($_POST['nonas']);$tgl=$tanggal;$branch=$kcabang;
$fieldss='a.noreks,a.tbunga,a.flunas,a.ketnas,a.status_klaim';
include '../config/_set_headerx.php';
$hasil=$result->query_lap("SELECT id,pokok,bunga,adm,jumlah FROM $tabelSusulan WHERE norek='$norek' AND kdtran=111 ORDER BY norek LIMIT 1");$row=$result->row($hasil);
$id=$row['id'];$saldoa=$row['pokok'];$blunas=$row['bunga'];$alunas=$row['adm'];$jumlunas=$saldoa+$blunas+$alunas;$norekl='';$kode_cair=1;
?>
	...
	<table width="100%">
		...
		<tr>
			...
			<select name="kode_cair" id="kode_cair" class="combobox">
				<?php $huruf = array("GIRO BTPN","PP ANGSURAN","UANG MUKA");$i = 0;
				while($i < 3){
					if($kode_cair== $i){
