...
if($branch=='0111'){
	$result->query_x1("CREATE TEMPORARY TABLE $tabelm SELECT branch,norek,kdtran,angsurke,SUM(pokok) as pokok,SUM(bunga) as bunga,SUM(adm) AS adm,SUM(jumlah) AS jumlah FROM $tabelr WHERE kdtran=777 GROUP BY norek ORDER BY norek");
}else{
	$result->query_x1("CREATE TEMPORARY TABLE $tabelm SELECT branch,norek,kdtran,angsurke,SUM(pokok) as pokok,SUM(bunga) as bunga,SUM(adm) AS adm,SUM(jumlah) AS jumlah FROM $tabelr WHERE branch='$branch' AND kdtran=777 GROUP BY norek ORDER BY norek");
}
if($ada==TRUE){
	if($produk=='9'){
