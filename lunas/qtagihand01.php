<?php $tgl1=$result->c_d($_GET['tgl1']);$tgl2=$result->c_d($_GET['tgl2']);$branch=$result->c_d($_GET['branch']);$kdkop =$result->c_d($_GET['kdkop']);$kdsales = $result->c_d($_GET['kdsales']);$pilih=$result->c_d($_GET['p']);$xt=date_angka(date_sql($tgl2));$tabel=$tabel_transaksi.$xt;
if($branch=='0111'){
	$hasil=$result->query_x1("CREATE TEMPORARY TABLE $userid SELECT branch,norek,oper,tanggal,SUM(IF(kdkredit=10,jumlah,0)) AS pokok,SUM(IF(kdkredit=20,jumlah,0)) AS bunga,SUM(IF(kdkredit=30,jumlah,0)) AS adm,SUM(IF(kdkredit=40,jumlah,0)) AS denda,SUM(IF(kdkredit=50,jumlah,0)) AS finalti,IF(SUBSTR(kdtran,1,1)<=2,'K','M') AS mutasi FROM $tabel  WHERE substr(jtransaksi,1,1)=3 AND tanggal>='$tgl1' AND tanggal<='$tgl2' GROUP BY norek ORDER BY norek");
}else{
	$hasil=$result->query_x1("CREATE TEMPORARY TABLE $userid SELECT branch,norek,oper,tanggal,SUM(IF(kdkredit=10,jumlah,0)) AS pokok,SUM(IF(kdkredit=20,jumlah,0)) AS bunga,SUM(IF(kdkredit=30,jumlah,0)) AS adm,SUM(IF(kdkredit=40,jumlah,0)) AS denda,SUM(IF(kdkredit=50,jumlah,0)) AS finalti,IF(SUBSTR(kdtran,1,1)<=2,'K','M') AS mutasi FROM $tabel  WHERE substr(jtransaksi,1,1)=3 AND tanggal>='$tgl1' AND tanggal<='$tgl2' AND branch='$branch' GROUP BY norek ORDER BY norek");
}
if($ada==TRUE){
	if($kdsales=='9'){
