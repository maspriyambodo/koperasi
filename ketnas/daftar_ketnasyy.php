...
while ($row=$result->row($hasil)){
	$kketnas=$row['kketnas'];
	include '../parameter/ket_kketnas.php';
	if ($vbayar!=$row['produk']){ 
		if($no>1){ ?>
			...
			$jml_nomi=0;$jml_sawal=0;$jml__kdebet=0;$jml_kkredit=0;$jml_mdebet=0;$jml_mkredit=0;$jml_sakhir=0;$jml_loan=0;$grand_nomi=0;$grand_sawal=0;$grand_kdebet=0;$grand_kkredit=0;$grand_mdebet=0;$grand_mkredit=0;$grand_sakhir=0;$grand_loan=0;
		}?>
	...
	<tr>
		...
		<td align="right">
			<?php 
				$kdpremi='TIDAK DI ASURANSIKAN';
				if($row['kdpremi']!='9'){
					$kdpremi='DI ASURANSIKAN';
				}
				echo $kketnasx .' / '.$kdpremi; 
			?>
		</td>
	</tr> <?php
