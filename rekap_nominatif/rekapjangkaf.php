<thead>
	<tr class="td" bgcolor="#e5e5e5">
		<th rowspan="2">NO</th>
		<th rowspan="2">JANGKA WAKTU / SUKU BUNGA</th>
	...
	while ($row = $result->row($hasil)) {
		...
		if ($vbayar<>$row['produk']){
			...
			if($kolekx<>$row['kolek']){ ?>
				<tr>
				<td>&nbsp;</td>
				<td colspan="9"><strong>KOLEKTIBILITAS : <?php echo $xkolek;?></strong></td>
				</tr><?php
			}
			$jsaldo_awal=0;$jkas_debet=0;$jkas_kredit=0;$jmemo_debet=0;$jmemo_kredit=0;$jsaldo_akhir=0;$jorg=0;
		}else{
		...
		<tr>
			<td><?php echo $no; ?></td>
			<td><?php echo 'JW '.$row['jangka'].'  '.$xkdkop.' / '.$row['suku'].' %';?></td>
		...
		$vbayar=$row['produk'];
		$vlunas=$row['jangka'].' - '.$row['suku'];
		$vnmbayar=$row['jangka'].'  '.$xkdkop.' / '.$row['suku'].' %';
		$kolekx=$row['kolek'];
		$no++;
	}
