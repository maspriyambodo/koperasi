...
$(document).ready(function () {
	...
	$('#masuk').submit(function () {
		...
		var saldo_akhir=new Number($("#saldo_akhir").val().replace(/\./g, ''));
		if(jumlah>saldo_akhir){
			alert('Jumlah Lebih Besar Dari Saldo Akhir Bloter ID !');
			return false;
		}
		$.ajax({
			type: 'POST',
			url: './bloter/tbltellerins.php',
			data: $(this).serialize(),
			beforeSend: function () {
				$('#loading').show();
			},
...
function goKembali() {
	$('#innertub').load('tbltellerin.php');
}
</script> 
<?php
$branch=$kcabang;
$bloterid=clean($_POST['bloterid']); 
$result = $mysql->query("SELECT id,branch,userid,bloterid,saldo_awal,mutasi_debet,mutasi_kredit,saldo_akhir,valid,date_open,date_expire FROM tbl_bloter WHERE branch='$branch' AND bloterid='$bloterid' LIMIT 1");include '../pesanerra.php';
if (mysqli_num_rows($result)<1){ 
	$xp='ID Bloter Tidak Ditemukan...?';include '../pesan.php';
}
$row=$result->fetch_array(MYSQLI_ASSOC);
if($row['valid']==1){
	$xp='Bloter ID Sudah Disable... ?';include '../pesan.php';
}
$useridx=$row['userid'];
...
$keterangan='Penarikan Uang Pada Bloter '.$bloterid.' User '.$useridx;?>
<form id="masuk" name="masuk" method="POST" action="" >
	<table align="center">
		...
		<tr>	<td>Saldo Kas</td>
			<td>: <input style="text-align:right;" name="saldo_kas" type="text" id="saldo_kas" size="25" value="<?php echo number_format($saldo_kas);?>" readonly/></td>
		</tr>
		<tr><td>ID Bloter Cash Out</td>
			<td>: <input name="bloterid" type="text" id="bloterid" size="25"  value="<?php echo $bloterid;?>" readonly/></td>
		</tr>
		<tr><td>User ID Cash Out</td>
			<td>: <input name="useridx" type="text" id="useridx" size="25"  value="<?php echo $useridx;?>" readonly/></td>
		</tr>
		<tr><td>Saldo Bloter Cash Out</td>
			<td>: <input style="text-align:right;" name="saldo_akhir" type="text" id="saldo_akhir" size="25" value="<?php echo number_format($saldo_akhir);?>" readonly/></td>
		</tr>
		<tr><td>Tanggal</td>
